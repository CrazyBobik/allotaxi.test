...
class K_TreeQuery
{
    ...
    public static function searcheByName($sercheNode, $nodeName, $type = false)
    {
        ...
        $result = $query->q('SELECT `tr`.*, `ty`.* FROM `tree` AS `tr` LEFT JOIN `type_'.$type.'` AS `ty` ON tr.tree_id  = ty.type_'.$type.'_id  WHERE tr.tree_lkey >= '.(int)$parentNodeData['tree_lkey'].' AND tr.tree_rkey <= '.(int)$parentNodeData['tree_rkey'].' AND tr.tree_name = "'.$nodeName.'"');
    ...
    public static function gNodes($opt, $limit = false, $parentNodeData = false)
    {
        ...
        $opt = array_merge(array (
            ...
            "group" => false,
            "order" => false,
            "more" => array ('cascade' => false, "childs" => false, "test" => false, 'meta', 'count' => false, 'orderby' => "ASC", "aliases" => false)
        ), $opt);
