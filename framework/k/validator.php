...
class K_Validator implements K_Model_IValidator {
    ...
    * @param $dictionary['user_name']='Имя пользователя'
    * @rerun $errors = array([1]=>array('name'=>'Имя пользователя','error'=>'Ошибка вылидации'))
    */ 
    public function getErrorsD($dictionary) {
       
       
         foreach($this->errors as $k=>$v){   
         
                   $errorArr['label']=$dictionary[$k];
                   $errorArr['error']=$v;
                   $errors[$k]=$errorArr;
             }
 	    return $errors;
